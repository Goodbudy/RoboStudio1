...
private:
    ...
    cv::Mat laserScanToMat(const sensor_msgs::msg::LaserScan::SharedPtr& scan) {
        ...
        //convert laser scan into cartesian form - laserscan gives range and angle. - figure out points in x and y and push into an empty image.
        //iterate over all of laser scan and convert into cartesian.
    ...
    void calculateYawChange() {
        ...
        //determine how much a robot has turned by comparing images
    ...
    double angle_difference_; //between each image.
    double relative_orientaion_ = 0.0; //from first image

    //for sprint 2 we capture image (laserscan) and compare it to map(lab5) and then try rotate robot to match map.
};
